...
function parseIntoFiles(json) {
    return new Promise((resolve, reject) => {
        let modules = new Utils.Dictionary(), parser = new NamespaceParser(json, modules);
        resolve(modules);
    });
}
exports.parseIntoFiles = parseIntoFiles;
class NamespaceParser {
     ...
     */
    constructor(sourceNode, targetModules) {
    ...
    findNodeInMainModules(nodeName) {
        let found = false, mainModule = this.targetModules[Config.mainModule], node = mainModule, spaceNames = TSD.IDeclaration.namespaces(nodeName, true), indexEnd = (spaceNames.length - 1);
