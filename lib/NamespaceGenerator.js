 ...
 *
 * */
function generate(moduleNodes, declarationModules) {
    return new Promise((resolve, reject) => {
        const globalsNamespace = new TSD.ModuleDeclaration('globals');
        const globalsModule = Utilities.path(Utilities.parent(Config.mainModule), 'globals');
        ...
        for (const moduleNode in moduleNodes) {
            declarationModules[moduleNode] = new Generator(moduleNode, moduleNodes[moduleNode], globalsNamespace, mainNamespace).moduleNamespace;
        }
        declarationModules[globalsModule] = globalsNamespace;
        if (!declarationModules[Config.mainModule]) {
            reject(new Error('Main module missing: ' + Config.mainModule + '; ' +
                'found only: ' + Object.keys(declarationModules).join(', ')));
        }
        moveReferenceDeclarations(declarationModules, referenceDictionary);
        updateImports(declarationModules);
        resolve(declarationModules);
    });
}
function moveReferenceDeclarations(declarationModules, referenceDictionary) {
    const mainNamespace = declarationModules[Config.mainModule];
    const moveReferences = (declaration) => {
...
function save(declarationsModules) {
    ...
    return new Promise((resolve, reject) => {
        ...
        Object
            ...
            savePromises.push(Utilities.save((module + '.src.d.ts'), declarations.replace(mainModuleRegExp, '$1.src$2')));
        });
        Promise
            .all(savePromises)
            .then(() => resolve())
...
function updateImports(declarationModules) {
    const updateImports = (modulePath, module, declaration = module) => {
...
class Generator {
    ...
    static getNormalizedDoclet(sourceNode) {
        ...
        if (doclet.parameters) {
            ...
            Object
                ...
                if (parameterDescription) {
                    parameterDescription = Utilities.removeLinks(parameterDescription, removedLinks);
                    parameterDescription = Utilities.transformLists(parameterDescription);
        ...
        if (doclet.return) {
            ...
            if (returnDescription) {
                returnDescription = Utilities.removeLinks(returnDescription, removedLinks);
                returnDescription = Utilities.transformLists(returnDescription);
        ...
        if (!Config.withoutLinks && removedLinks.length > 0) {
            let see = [];
            removedLinks.forEach(link => see.push(...Utilities.urls(link)));
            if (see.length > 0) {
     ...
     *
     * */
    constructor(modulePath, moduleNode, globalsNamespace, mainNamespace) {
        ...
        if (this.modulePath === Config.mainModule) {
            ...
            this.moduleNamespace.imports.push(('import * as globals from "' + Utilities.relative(modulePath, Config.mainModule.replace(/highcharts$/, 'globals'), true) + '";'), ('import * as Highcharts from "' + Utilities.relative(modulePath, Config.mainModule, true) + '";'));
            ...
            this.moduleNamespace.imports.push(('import * as _Highcharts from "' + Utilities.relative(modulePath, Config.mainModule, true) + '";'));
            if (!Config.withoutFactory && factoryDeclaration) {
                ...
                this.moduleNamespace.exports.push('export default _Highcharts;');
            }
        }
    ...
    generate(sourceNode, targetDeclaration = this._moduleNamespace) {
        ...
        switch (kind) {
            default:
                console.error('Unknown kind: ' + kind, this.modulePath, JSON.stringify(sourceNode));
                break;
            case 'class':
    ...
    generateClass(sourceNode, targetDeclaration) {
        ...
        if (doclet.types) {
            let mergedTypes = Utilities.uniqueArray(declaration.types, doclet.types.filter(type => type !== type.toLowerCase()));
    ...
    generateExternalModule(sourceNode, targetDeclaration) {
        ...
        declaration = new TSD.ExternalModuleDeclaration(this._mainNamespace.name, Utilities.relative(this.modulePath, Config.mainModule, true));
        if (doclet.isGlobal) {
    ...
    generateFunction(sourceNode, targetDeclaration) {
        ...
        if (doclet.return) {
            ...
            if (doclet.return.types) {
                let mergedTypes = Utilities.uniqueArray(declaration.types, doclet.return.types);
                declaration.types.length = 0;
                declaration.types.push(...mergedTypes);
            }
        }
    ...
    generateFunctionInterface(sourceNode, targetDeclaration) {
        ...
        if (doclet.return) {
            ...
            if (doclet.return.types) {
                let mergedTypes = Utilities.uniqueArray(functionDeclaration.types, doclet.return.types);
    ...
    generateProperty(sourceNode, targetDeclaration) {
        ...
        if (doclet.types) {
            let mergedTypes = Utilities.uniqueArray(declaration.types, doclet.types);
            declaration.types.length = 0;
            declaration.types.push(...mergedTypes);
        }
        if (doclet.requires) {
    ...
    generateType(sourceNode, targetDeclaration) {
        ...
        if (doclet.types) {
            let mergedTypes = Utilities.uniqueArray(declaration.types, doclet.types.filter(type => !sourceNode.children ||
