...
const COPYRIGHT = 'Copyright (c) Highsoft AS. All rights reserved.';
function generate(namespaceModules) {
    return new Promise(resolve => {
        ...
        Object
            ...
            declarationsGenerators[module] = new Generator(globals, namespace, productsModules[module]);
        ...
        Object
            .keys(productsModules)
            .forEach(module => declarations[module] = (declarationsGenerators[module].namespace));
        resolve(declarations);
    });
}
exports.generate = generate;
function generateModule(modulePath) {
    const moduleGlobal = new TSD.ModuleDeclaration();
    moduleGlobal.description = COPYRIGHT;
    moduleGlobal.imports.push(('import * as Highcharts from "' + Utils.relative(modulePath, Config.mainModule, true) + '";'));
    moduleGlobal.exports.push('export default factory;');
    const factoryDeclaration = new TSD.FunctionDeclaration('factory');
    factoryDeclaration.description = ('Adds the module to the imported Highcharts namespace.');
    const factoryParameterDeclaration = new TSD.ParameterDeclaration('highcharts');
...
function save(cliFeedback, namespaceModules, optionsModules) {
    ...
    const modularProductsModules = {};
    Object
        .keys(modularProducts)
        .forEach(product => modularProductsModules[modularProducts[product]] = product);
    const products = Config.products;
    const productsModules = {};
    Object
        .keys(products)
    ...
    Object
        ...
        if (optionsModules[module]) {
            declarations.addChildren(...optionsModules[module].removeChildren());
        }
    });
    const filePromises = [];
    Object
        ...
        if (modularProductsModules[module]) {
            const externalNamespace = new TSD.ExternalModuleDeclaration(mainModule.name, '../highcharts');
            externalNamespace.addChildren(...filterChildDeclarations(namespaceModules[products[modularProductsModules[module]]].getChildren(), mainModule.getChildren(), modularProductsModules[module])
        ...
        filePromises.push(Utils.save(module + '.d.ts', declarations.toString()), Utils.save(module + '.src.d.ts', declarations
            .toString()
...
exports.save = save;
function filterChildDeclarations(declarationsToFilter, referenceDeclarations, product) {
    const filteredChildDeclarations = [];
    declarationsToFilter.forEach(declaration => {
        ...
        if (foundReference instanceof TSD.ClassDeclaration ||
            ...
            const newDeclaration = new TSD.InterfaceDeclaration(declaration.name);
            newDeclaration.addChildren(...filterChildDeclarations(declaration.getChildren(), foundReference.getChildren(), product)
        ...
        if (foundReference instanceof TSD.TypeDeclaration &&
            ...
            const newDeclaration = new TSD.TypeDeclaration(Utils.capitalize(product) + 'SeriesOptionsType');
...
function filterInvalidTypes(namespace) {
    const namespaceTypes = namespace.getChildrenNames(true);
    const removedTypes = [];
    function filterTypes(declaration) {
        ...
        typeNames
            ...
            validTypes = Utils.uniqueArray(declaration.types.map(type => type.replace(new RegExp((name.replace('.', '\\.') +
                '(?=' +
...
function filterModularProductModule(mainNamespace, productNamespace, modularProductNamespace) {
    const mainName = mainNamespace.fullName;
    const mainTypes = mainNamespace.getChildrenNames();
    const modularNamespace = new TSD.ExternalModuleDeclaration('Highcharts', '../highcharts');
    modularProductNamespace.addChildren(modularNamespace);
    console.log(mainNamespace.getChildren().length, productNamespace.getChildren().length);
    modularNamespace.addChildren(...filterUnknownChildDeclarations(productNamespace, mainNamespace));
    console.log('Cloned', modularNamespace.getChildrenNames().join(', '));
    return modularProductNamespace;
}
function filterUnknownChildDeclarations(declarationToFilter, referenceDeclaration) {
    let referenceChildNames;
    ...
    childrenToFilter
        ...
        const different = referenceDeclaration
            ...
            const unknownChildChildren = filterUnknownChildDeclarations(childToFilter, referenceChild);
        ...
        if (childToFilter instanceof TSD.FunctionDeclaration) {
            const unknownChild = new TSD.FunctionDeclaration(childToFilter.name);
            unknownChild.setParameters(...childToFilter
                .getParameters()
                .map(parameter => parameter.clone()));
            unknownChildren.push(unknownChild);
        }
        else {
            const unknownChild = new TSD.InterfaceDeclaration(childToFilter.name);
            unknownChild.addChildren(...differences
...
function mergeDeclarations(targetDeclaration, sourceDeclaration) {
    if (!targetDeclaration.description) {
        targetDeclaration.description = sourceDeclaration.description;
    }
    targetDeclaration.types.push(...Utils.uniqueArray(targetDeclaration.types, sourceDeclaration.types));
    let existingChild = undefined, sourceChildren = sourceDeclaration.getChildren(), targetChildrenNames = targetDeclaration.getChildrenNames();
...
class Generator {
     ...
     *
     * */
    constructor(globals, namespace, product) {
     ...
     *
     * */
    generate(sourceNode, targetDeclaration = this._namespace, product = this._product) {
    ...
    */
    ...
    isMainMember(name, baseDeclaration = this._namespace) {
