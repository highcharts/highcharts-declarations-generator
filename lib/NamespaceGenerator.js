...
function generate(moduleNodes, optionDeclarations) {
    return new Promise(resolve => {
        ...
        Object
            .keys(moduleNodes)
            .forEach(module => declarationModules[module] = new Generator(module, moduleNodes[module], globalsNamespace, optionDeclarations).moduleNamespace);
...
class Generator {
     ...
     *
     * */
    constructor(modulePath, node, globalsNamespace, optionsNamespace) {
        ...
        if (this.modulePath === Config.mainModule) {
            ...
            this.moduleNamespace.imports.push(('import * as globals from "' + Utils.relative(modulePath, Config.mainModule.replace(/highcharts$/, 'globals'), true) + '";'), ('import * as Highcharts from "' + Utils.relative(modulePath, Config.mainModule, true) + '";'));
    ...
    isMainMember(name, baseDeclaration = this.mainNamespace) {
        ...
        name = TSD.IDeclaration
            ...
            .filter(type => !Utils.isCoreType(type))[0];
 ...
 *
 * */
Generator.OPTION_TYPE = (/^Highcharts.\w+(?:CallbackFunction|Object|Options|Value)$/);
Generator.temporaryDictionary = {};
