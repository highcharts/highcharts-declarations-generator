...
class Parser extends Object {
    ...
    cloneNodeInto(sourceNode, targetNode) {
        let sourceDoclet = sourceNode.doclet, sourceMeta = sourceNode.meta, targetDoclet = targetNode.doclet, targetMeta = targetNode.meta, targetName = (targetMeta.fullname || targetMeta.name);
    ...
    completeNodeExtensions(node) {
        let nodeChildren = node.children, nodeExcludes = (node.doclet.exclude || []), nodeExtends = (node.doclet.extends || '');
        if (nodeExtends) {
            ...
            nodeExtends
                ...
                .forEach(xName => {
                if (xName.indexOf('{') > -1) {
                    console.error('Extends: ' +
                        'Curly brackets notation should be avoided.', xName);
                    xName = xName.substr(1, xName.length - 2);
                }
                let xNode = this.findNode(xName);
                if (!xNode) {
                    console.error('Extends: Node ' + xName + ' not found.', node.meta.fullname || node.meta.name);
                    return;
                }
