...
class IDeclaration extends Object {
    ...
    renderTypes(useParentheses = false, filterUndefined = false, filterFunctions = false) {
        let root = this.root, rootName = root && root.name, types = this.types.slice();
        if (rootName) {
 ...
 */
IDeclaration.PATH_ELEMENTS = (/^([\.\/\\]*)([\w\.\/\\]*)(\w*)([\w\.]*)$/);
/**
...
class IExtendedDeclaration extends IDeclaration {
    ...
    setParameters(...declarations) {
        let parameters = this._parameters, name = '';
        declarations.forEach(declaration => {
            if (declaration.parent) {
            ...
            name = declaration.name;
            if (parameters[name]) {
                throw new Error('Parameter declaration with the name "' +
...
class ClassDeclaration extends IExtendedDeclaration {
    ...
    toString(indent = '') {
        ...
        let childIndent = indent + '    ', renderedChildren = this.renderChildren(childIndent), renderedClass = 'class ' + this.name, renderedDescription = this.renderDescription(indent);
        if (this.hasTypes) {
...
class FunctionDeclaration extends IExtendedDeclaration {
     ...
     */
    toString(indent = '') {
        let renderedDescription = this.renderExtendedDescription(indent), renderedFunction = this.name, renderedParameters = this.renderParameterBrackets(), renderedScope = this.renderScopePrefix(), renderedReturn = this.renderTypes(true, true, true);
...
class InterfaceDeclaration extends IDeclaration {
     ...
     */
    toString(indent = '') {
        let childIndent = indent + '    ', renderedChildren = this.renderChildren(childIndent), renderedDescription = this.renderDescription(indent, true), renderedInterface = this.name;
        if (!this.childOfSpace()) {
            renderedInterface += ': ';
        }
        else {
...
class ModuleDeclaration extends IDeclaration {
     ...
     */
    toString(indent = '') {
        let childIndent = indent + '    ', renderedChildren = this.renderChildren(childIndent), renderedDescription = this.renderDescription(indent), renderedModule = 'module "' + this.path + '"';
 ...
 * @extends IDeclaration
 */
class ModuleGlobalDeclaration extends IDeclaration {
    /* *
     *
     *  Constructor
     *
     ...
     */
    toString() {
        let renderedChildren = this.renderChildren(''), renderedDescription = this.renderDescription(''), renderedExports = this.renderExports(), renderedImports = this.renderImports();
        if (renderedDescription) {
...
exports.ModuleGlobalDeclaration = ModuleGlobalDeclaration;
/**
...
class NamespaceDeclaration extends IDeclaration {
     ...
     */
    toString(indent = '') {
        let childIndent = indent + '    ', renderedChildren = this.renderChildren(childIndent), renderedDescription = this.renderDescription(indent), renderedNamespace = (this.name === 'external:' ?
            'global' :
...
class ParameterDeclaration extends IDeclaration {
     ...
     */
    toString() {
        let renderedParameter = this.name, renderedTypes = this.renderTypes(true, true, true);
        if (this.isOptional) {
            renderedParameter += '?';
        }
        if (this.isVariable) {
...
class PropertyDeclaration extends IDeclaration {
     ...
     */
    toString(indent = '') {
        let childIndent = indent + '    ', renderedMember = this.name.replace(':', ': ');
        // special treatment for indexers
        if (renderedMember[0] === '[' &&
            renderedMember.indexOf(' ') > -1) {
            let root = this.root, rootName = root && root.name;
            if (rootName) {
        ...
        if (this.hasChildren) {
            ...
            renderedMember += (': ' + this.renderTypes(true, this.isIndexer, true) + ';');
        }
        else {
            renderedMember += ': any;';
        }
        renderedMember = this.renderScopePrefix() + renderedMember;
        return (this.renderDescription(indent, true) +
...
class TypeDeclaration extends IDeclaration {
     ...
     */
    toString(indent = '') {
        let childIndent = indent + '    ', renderedChildren = this.renderChildren(childIndent), renderedType = this.renderTypes(true);
        if (!renderedType) {
            renderedType = 'any';
        }
        if (this.hasChildren) {
        ...
        return (this.renderDescription(indent) +
            indent + renderedType);
    }
}
