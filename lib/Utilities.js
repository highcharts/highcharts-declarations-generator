...
const CWD = process.cwd().split(node_path_1.sep).join(node_path_1.posix.sep);
...
function base(filePath) {
    let slashIndex = filePath.lastIndexOf(node_path_1.posix.sep), pointIndex = filePath.indexOf('.', slashIndex);
...
function copy(sourceFilePath, targetFilePath) {
    return node_fs_1.promises
        .mkdir(node_path_1.posix.dirname(targetFilePath), { recursive: true })
        .then(() => node_fs_1.promises.copyFile(sourceFilePath, targetFilePath))
        .then(() => targetFilePath);
}
...
function load(filePath) {
    filePath = node_path_1.posix.join(CWD, filePath);
    return node_fs_1.promises
        .readFile(filePath)
...
function parent(childPath) {
    return node_path_1.posix.dirname(childPath);
}
function path(...pathes) {
    return node_path_1.posix.join(...pathes);
}
...
function relative(fromPath, toPath, moduleMode = false) {
    ...
    if (moduleMode || node_path_1.posix.extname(fromPath)) {
        fromDirectory = node_path_1.posix.dirname(fromDirectory);
        isFromFile = true;
    }
    if (moduleMode || node_path_1.posix.extname(toPath)) {
        toDirectory = node_path_1.posix.dirname(toDirectory);
        isToFile = true;
    }
    let relativePath = node_path_1.posix.relative(fromDirectory, toDirectory);
    if (moduleMode &&
        relativePath[0] !== '.') {
        if (relativePath[0] !== node_path_1.posix.sep) {
            relativePath = node_path_1.posix.sep + relativePath;
        }
        relativePath = '.' + relativePath;
    }
    if (isToFile) {
        if (relativePath &&
            relativePath[relativePath.length - 1] !== node_path_1.posix.sep) {
            relativePath += node_path_1.posix.sep;
        }
        return relativePath + node_path_1.posix.basename(toPath);
    }
    else {
        if (relativePath &&
            relativePath[relativePath.length - 1] === node_path_1.posix.sep) {
...
function save(filePath, fileContent) {
    filePath = node_path_1.posix.join(CWD, filePath);
    return node_fs_1.promises
        .mkdir(node_path_1.posix.dirname(filePath), { recursive: true })
        .then(() => node_fs_1.promises.writeFile(filePath, fileContent))
        .then(() => filePath);
}
