...
function copy(sourceFilePath, targetFilePath) {
    return new Promise((resolve, reject) => {
        MkDirP(path_1.posix.dirname(targetFilePath), error => {
            if (error) {
                reject(error);
                return;
            }
    ...
    return new Promise((resolve, reject) => {
        try {
            FS
                ...
                const promisedFiles = entries.filter(entry => FS.statSync(entry).isFile());
                Promise
                    .all(filesPromises)
                    .then(results => {
...
function save(filePath, fileContent) {
    return new Promise((resolve, reject) => {
        filePath = path_1.posix.join(CWD, filePath);
        MkDirP(path_1.posix.dirname(filePath), err => {
            if (err) {
                reject(err);
                return;
            }
