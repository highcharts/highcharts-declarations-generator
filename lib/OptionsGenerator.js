...
class Generator {
    ...
    static getNormalizedDoclet(node) {
        ...
        if (doclet.type && doclet.type.names) {
            doclet.type.names = Utils.uniqueArray(doclet.type.names.map(type => Config.mapType(type)));
        }
        else {
    ...
    generateInterfaceDeclaration(sourceNode) {
        ...
        let doclet = Generator.getNormalizedDoclet(sourceNode), name = Generator.getCamelCaseName(sourceNode.meta.fullname || sourceNode.meta.name || ''), declaration = new TSD.InterfaceDeclaration(sourceNode.doclet.declare || name), children = Utils.Dictionary.values(sourceNode.children);
    ...
    generateLiteralTypeDeclarations(sourceDeclaration = this._namespace) {
        ...
        if (sourceDeclaration instanceof TSD.PropertyDeclaration &&
            ...
            let name = ('Options' + Utils.capitalize(sourceDeclaration.name) + 'Value');
    ...
    generatePropertyDeclaration(sourceNode, targetDeclaration) {
        ...
        if (Object.keys(sourceNode.children).length > 0) {
            ...
            sourceNode.doclet.type.names = Utils.uniqueArray(sourceNode.doclet.type.names);
        }
        ...
        if (!isValueType && doclet.type) {
            const mergedTypes = Utils.uniqueArray(declaration.types, doclet.type.names);
            declaration.types.length = 0;
            declaration.types.push(...mergedTypes);
        }
        if (!declaration.parent) {
    ...
    generateSeriesDeclaration() {
        ...
        let seriesTypeDeclaration = new TSD.TypeDeclaration('SeriesOptionsType');
    ...
    generateSeriesTypeDeclaration(sourceNode, targetDeclaration) {
        ...
        typePropertyDeclaration.description = ('(' + Object.keys(Config.products).map(Utils.capitalize).join(', ') + ') ' +
    ...
    generateTypeDeclaration(name, types, description) {
        ...
        if (existingDeclaration instanceof TSD.TypeDeclaration) {
            ...
            const mergedTypes = Utils.uniqueArray(existingDeclaration.types, types);
