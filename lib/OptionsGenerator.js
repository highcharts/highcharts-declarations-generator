...
function declare(parsedOptions) {
    return new Promise(resolve => Object
        .keys(Config.products)
        .map(product => (new Generator(product, parsedOptions[product])).namespace));
}
...
class Generator {
     ...
     *
     * */
    constructor(product, parsedOptions) {
    ...
    generatePropertyDeclaration(sourceNode, targetDeclaration) {
        ...
        if (Object.keys(sourceNode.children).length > 0) {
            const interfaceDeclaration = this.generateInterfaceDeclaration(sourceNode);
