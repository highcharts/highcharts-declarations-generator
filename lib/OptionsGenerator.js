    ...
    for (const child of optionsNamespace.getChildren().slice()) {
        ...
        if (seriesTypes.includes(type) || seriesTypes.includes(type2)) {
            ...
            if (!declarationModules[modulePath]) {
                ...
                declarationModules[modulePath].imports.push(...(optionsNode.doclet.requires || ['highcharts']).map(require => {
                    if (require === 'highcharts') {
                        return ('import * as Highcharts from "' +
                            Utilities.relative(modulePath, Config.mainModule, true) +
                            '";');
                    }
                    return ('import "' +
                        Utilities.relative(modulePath, require.replace('highcharts/', ''), true) +
                        '";');
                }));
