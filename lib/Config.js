 ...
 *
 * */
const MAP_TYPE_MINIARRAY = /Array<((?:[^<>\[\]]+|<[^<>\[\]]+>),(?:[^<>\[\]]+|<[^<>\[\]]+>)+)>/;
...
config.treeNamespaceJsonFile = (config.treeNamespaceJsonFile || '').replace(/\//g, Path.sep);
config.treeOptionsJsonFile = (config.treeOptionsJsonFile || '').replace(/\//g, Path.sep);
